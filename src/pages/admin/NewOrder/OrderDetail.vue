    ...
    <v-container fluid>
        <v-card class="pa-4">
            ...
            <v-row align="center" justify="space-between" class="px-4 mb-4">
                <v-col cols="auto">
                    <span class="text-h5">CHI TIẾT ĐƠN HÀNG {{ userOrderInfor.order_id }}</span>
                </v-col>
                <v-col cols="auto">
                    <v-chip
                        :color="getStatusColor(userOrderInfor.order_status)"
                        class="px-4"
                    >
            ...
            <v-row>
                <v-col cols="12" md="6" v-for="field in customerFields" :key="field.label">
                    <v-row align="center">
                        ...
                        <v-col cols="12" md="8">
                            <v-text-field :model-value="userOrderInfor[field.key]" readonly density="compact"
                ...
                <v-col cols="12">
                    <v-row align="center">
                        ...
                        <v-col cols="12" md="10">
                            <v-text-field :model-value="userOrderInfor.order_note" readonly density="compact"
                                variant="outlined" hide-details></v-text-field>
                        </v-col>
                    </v-row>
                </v-col>
            ...
            <v-card class="mt-6">
                ...
                <v-data-table :headers="headers" :items="products_infors" density="compact"
                    class="elevation-1">
                    <template v-slot:item.topping_name="{ item }">
                        <div v-if="item.toppings && item.toppings.length > 0">
                            <div v-for="(topping, index) in item.toppings" :key="index" class="d-flex">
                                <span class="ml-4">{{ topping.name }} </span>
                                <span> +{{ topping.price }}đ</span>
                            </div>
                        </div>
            ...
            <v-card class="mt-6 pa-4">
                ...
                <v-row align="center" class="mb-2">
                    ...
                    <v-col cols="4" class="text-right">
                        <span class="text-subtitle-1 error--text">- {{ formattedPrice(userOrderInfor.order_discount_amount) }} đ</span>
                    </v-col>
                </v-row>
                ...
                <v-row align="center">
                    ...
                    <v-col cols="4" class="text-right">
                        <span class="text-h6 font-weight-bold primary--text">{{ formattedPrice(payment_infor[0].total) }} đ</span>
                    </v-col>
                </v-row>
            ...
            <v-row justify="center" class="mt-6 pa-4" spacing="4">
                <v-col cols="auto">
                    <v-btn color="success" size="large" @click="handleGiaoHang">
                        BẮT ĐẦU GIAO HÀNG
                    </v-btn>
                </v-col>
                <v-col cols="auto">
                    <v-btn color="error" size="large" @click="handleCancel">
                        HỦY
                    </v-btn>
                </v-col>
            </v-row>
...
export default {
    ...
    data() {
        return {
            ...
            headers: [
                ...
                { title: 'TÊN SẢN PHẨM', key: 'name', align: 'center', width: '30%' },
                { title: 'Topping', key: 'topping_name', align: 'center', width: '40%' },
                { title: 'Ghi chú', key: 'item_note', align: 'center', width: '10%' },
    ...
    methods: {
        ...
        async getOrderInfo() {
            try {
                const { data } = await orderAPI.getOrderInfo({ order_code: this.currentCODE });
        ...
        async handleGiaoHang() {
            try {
                await orderAPI.acceptOrder({ order_code: this.currentCODE });
        ...
        async handleCancel() {
            try {
                await orderAPI.cancelOrder({ order_code: this.currentCODE });
